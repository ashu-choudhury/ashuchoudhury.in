# Build stage: compile the single binary.
# Uses a pinned Go version for reproducible builds.
FROM golang:1.26-alpine AS build

# templ is only needed at generation time; the generated *_templ.go files
# are committed, so the build stage does not need templ at all.
WORKDIR /src

# Cache dependencies first (only re-runs on go.mod/go.sum changes).
COPY go.mod go.sum ./
RUN go mod download

# Copy the rest of the source and build.
COPY . .
RUN CGO_ENABLED=0 GOOS=linux go build -trimpath -ldflags="-s -w" -o /out/portfolio .

# Pre-create the data directory and hand it to the runtime user (uid 10001).
# Docker copies image contents AND ownership into a named volume on first
# mount, so a fresh `portfolio-data` volume is writable without extra steps.
RUN mkdir -p /out/storage/persisted && chown -R 10001:10001 /out/storage

# ---------------------------------------------------------------------------
# Runtime stage: scratch keeps the image tiny (~15 MB) and single-purpose.
FROM scratch

# Trusted CA bundle so S3 / outbound calls work.
COPY --from=build /etc/ssl/certs/ca-certificates.crt /etc/ssl/certs/

# The compiled binary, owned by the runtime user.
COPY --from=build --chown=10001:10001 /out/portfolio /portfolio

# The data directory, owned by the runtime user (see note above).
COPY --from=build --chown=10001:10001 /out/storage /storage

# The site (templates, CSS, htmx) is embedded in the binary; only the
# database lives on disk. Storage must be a volume so data survives
# restarts and can be backed up to S3.
VOLUME ["/storage"]

# Non-root for security: uid/gid 10001 (like distroless).
USER 10001:10001

ENV PORT=8080 \
    DB_PATH=/storage/persisted/portfolio.db \
    SITE_URL=https://ashuchoudhury.in

EXPOSE 8080
ENTRYPOINT ["/portfolio"]
